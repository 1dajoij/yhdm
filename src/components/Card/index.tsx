  ...
  return (
    <div className="card w-152px h-286px p-y-6px">
      <div className="w-100% relative card-img cursor-pointer">
        <Image preview={false} src={props.item.picUrl} width={152} height={228} className="rounded-6px" />
        ...
        <div className="mask sm-i absolute w-100% h-100% left-0 top-0 flex justify-center items-center rounded-6px">
          <Image preview={false} width={86} height={86} src={play}></Image>
        </div>
      </div>
      <div className="flex flex-wrap p-t-6px">
        <div>
          <span className="text-overflow max-w-100% text-15px font-500 cursor-pointer">
            {props.item.name}
          </span>
        </div>
        <span className="starring p-t-4px w-100% text-overflow text-13px color-#999">
